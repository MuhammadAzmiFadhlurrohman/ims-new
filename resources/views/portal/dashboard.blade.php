...
<body x-data="{
    ...
    {{-- ══════════════════════════════════════════════════════════════
         ── 3-TAB MAIN NAVIGATION BAR (CLEAN & MODULAR) ──
         ══════════════════════════════════════════════════════════════ --}}
    <div class="bg-[#0b1728]/80 border-b border-white/10 backdrop-blur-md sticky top-20 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between sm:justify-start gap-2 py-3 overflow-x-auto">
                ...
                <button 
                    ...
                    class="px-5 py-2.5 rounded-xl text-xs sm:text-sm transition-all flex items-center gap-2 shrink-0">
                    <span>📊</span>
                ...
                <button 
                    ...
                    class="px-5 py-2.5 rounded-xl text-xs sm:text-sm transition-all flex items-center gap-2 shrink-0">
                    <span>🎫</span>
                    <span>Menu Tiket &amp; Layanan</span>
                    @if($activeTickets->count() > 0)
                        <span class="px-2 py-0.5 rounded-full bg-amber-400 text-[#08111e] text-[10px] font-black">
                            {{ $activeTickets->count() }}
                        </span>
                    @endif
                </button>
                ...
                <button 
                    ...
                    class="px-5 py-2.5 rounded-xl text-xs sm:text-sm transition-all flex items-center gap-2 shrink-0">
                    <span>💳</span>
                    <span>Menu Tagihan &amp; Riwayat</span>
                    @if($hasArrears)
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-8">
            ...
            <div class="p-5 rounded-2xl bg-gradient-to-r from-amber-500/20 via-brand-500/20 to-emerald-500/20 border border-amber-500/40 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
                <div>
                    ...
                    <p class="text-slate-300">
                        Nomor Tiket Anda: <strong class="text-white font-mono bg-black/40 px-2 py-0.5 rounded">{{ session('ticket_created')['ticket_no'] }}</strong>. Tim Helpdesk &amp; Teknisi NOC akan segera memverifikasi laporan Anda.
                    </p>
                </div>
                <button @click="currentNav = 'tiket'" class="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-[#08111e] font-black text-xs shrink-0 shadow-md">
        ...
        <div x-show="currentNav === 'dashboard'" x-transition class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-8 glass-card rounded-3xl p-6 sm:p-8 relative overflow-hidden flex flex-col justify-between shadow-xl">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6 border-t border-white/10 text-xs">
                        <div>
                            <span class="text-[11px] text-slate-400 block mb-0.5 font-medium">Paket Langganan</span>
                            <strong class="text-white">{{ $currentPackage->name ?? ($subscription->package_code ?? 'Home Fiber 50M') }}</strong>
                        </div>
                        <div>
                            <span class="text-[11px] text-slate-400 block mb-0.5 font-medium">Kecepatan Fiber</span>
                            <strong class="text-cyan-400 font-bold">{{ $currentPackage->speed_mbps ?? 50 }} Mbps Simetris</strong>
                        </div>
                        <div>
                            <span class="text-[11px] text-slate-400 block mb-0.5 font-medium">Titik ODP Node</span>
                            <strong class="text-white">{{ $subscription->odp_code ?? 'ODP-BDG-BRAGA-01' }} (Port {{ $subscription->odp_port ?? '03' }})</strong>
                        </div>
                        <div>
                            <span class="text-[11px] text-slate-400 block mb-0.5 font-medium">No. WhatsApp</span>
                ...
                <!-- Quick Summary Card -->
                <div class="lg:col-span-4 glass-card rounded-3xl p-6 sm:p-8 flex flex-col justify-between border-brand-400/25 shadow-xl">
                    <div>
                        <div class="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
                            <span class="text-xs font-black text-slate-300 uppercase tracking-wider">STATUS BULAN INI</span>
                            @if(!$hasArrears)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div @click="currentNav = 'tiket'; activeTicketTab = 'gangguan'" class="glass-card rounded-3xl p-6 glass-card-hover cursor-pointer border border-rose-500/20">
                    <div class="w-12 h-12 rounded-2xl bg-rose-500/20 text-rose-400 flex items-center justify-center text-2xl mb-4">
                        🚨
                    </div>
                    <h4 class="font-heading text-lg font-black text-white mb-1">Laporkan Gangguan</h4>
                    <p class="text-xs text-slate-400 mb-4 leading-relaxed">LOS merah, internet lemot, atau kabel putus? Lapor langsung ke tim NOC.</p>
                    <span class="text-xs font-extrabold text-rose-400 flex items-center gap-1">Buat Laporan Baru &rarr;</span>
                </div>

                <!-- Quick Card: Upgrade / Downgrade -->
                <div @click="currentNav = 'tiket'; activeTicketTab = 'upgrade'" class="glass-card rounded-3xl p-6 glass-card-hover cursor-pointer border border-cyan-500/20">
                    <div class="w-12 h-12 rounded-2xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-2xl mb-4">
                        🚀
                    </div>
                    <h4 class="font-heading text-lg font-black text-white mb-1">Ubah Paket Bandwidth</h4>
                    <p class="text-xs text-slate-400 mb-4 leading-relaxed">Upgrade speed ke 100 Mbps atau 1 Gbps tanpa ganti modem router.</p>
                ...
                <div @click="currentNav = 'tagihan'" class="glass-card rounded-3xl p-6 glass-card-hover cursor-pointer border border-brand-500/20">
                    <div class="w-12 h-12 rounded-2xl bg-brand-500/20 text-brand-400 flex items-center justify-center text-2xl mb-4">
                        💳
                    </div>
                    <h4 class="font-heading text-lg font-black text-white mb-1">Riwayat &amp; Tagihan</h4>
                    <p class="text-xs text-slate-400 mb-4 leading-relaxed">Cek invoice bulan berjalan, riwayat bayar, dan status lunas bulanan.</p>
                    <span class="text-xs font-extrabold text-brand-400 flex items-center gap-1">Buka Menu Tagihan &rarr;</span>
                </div>
        ...
        {{-- ══════════════════════════════════════════════════════════════
             ...
             ══════════════════════════════════════════════════════════════ --}}
        <div x-show="currentNav === 'tiket'" x-cloak x-transition class="space-y-8">
            ...
            <div class="glass-card rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/10">
                <div class="mb-6">
                    ...
                    <p class="text-xs text-slate-400">
                        Pilih jenis permohonan yang Anda butuhkan. Laporan Anda akan langsung masuk ke sistem monitoring tim teknisi NOC.
                    </p>

                    <!-- Service Sub-Tabs -->
                    <div class="flex flex-wrap gap-2.5 mt-6 border-b border-white/10 pb-4">
                        <button @click="activeTicketTab = 'gangguan'" :class="{'bg-brand-600 text-white shadow-lg': activeTicketTab === 'gangguan', 'bg-white/5 text-slate-300 hover:bg-white/10': activeTicketTab !== 'gangguan'}" class="px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2">
                            <span>🚨 Laporkan Gangguan Jaringan</span>
                        </button>
                        <button @click="activeTicketTab = 'upgrade'" :class="{'bg-brand-600 text-white shadow-lg': activeTicketTab === 'upgrade', 'bg-white/5 text-slate-300 hover:bg-white/10': activeTicketTab !== 'upgrade'}" class="px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2">
                            <span>🚀 Request Upgrade / Downgrade</span>
                        </button>
                        <button @click="activeTicketTab = 'relokasi'" :class="{'bg-brand-600 text-white shadow-lg': activeTicketTab === 'relokasi', 'bg-white/5 text-slate-300 hover:bg-white/10': activeTicketTab !== 'relokasi'}" class="px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2">
                            <span>🏠 Pindah Alamat (Relokasi)</span>
                        </button>
                        <button @click="activeTicketTab = 'wifi'" :class="{'bg-brand-600 text-white shadow-lg': activeTicketTab === 'wifi', 'bg-white/5 text-slate-300 hover:bg-white/10': activeTicketTab !== 'wifi'}" class="px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2">
                ...
                <form action="{{ route('customer.ticket.submit') }}" method="POST" class="space-y-6 text-xs">
                    ...
                    <div x-show="activeTicketTab === 'gangguan'" x-cloak class="space-y-5">
                    ...
                    <div x-show="activeTicketTab === 'upgrade'" x-cloak class="space-y-5">
                    ...
                    <div x-show="activeTicketTab === 'relokasi'" x-cloak class="space-y-5">
                    ...
                    <div x-show="activeTicketTab === 'wifi'" x-cloak class="space-y-5">
                        ...
                        @foreach($tickets as $tkt)
                            <div class="glass-card rounded-2xl p-5 border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                ...
                                <div class="sm:text-right shrink-0 text-xs">
                                    <span class="text-[11px] text-slate-400 block font-medium">PIC Teknisi:</span>
        ...
        {{-- ══════════════════════════════════════════════════════════════
             ...
             ══════════════════════════════════════════════════════════════ --}}
        <div x-show="currentNav === 'tagihan'" x-cloak x-transition class="space-y-8">
            
            <!-- Ringkasan Tagihan Bulan Berjalan -->
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                
                <div class="lg:col-span-8 glass-card rounded-3xl p-6 sm:p-8 border border-brand-400/25 shadow-xl flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between pb-4 border-b border-white/10 mb-5">
                            <div>
                                ...
                                <h3 class="font-heading text-xl font-bold text-white mt-1">{{ $currentPackage->name ?? 'Paket Internet Fiber' }}</h3>
                            </div>
                            @if(!$hasArrears)
                        ...
                        <div class="space-y-3 text-xs mb-6">
                            <div class="flex items-center justify-between text-slate-300">
                                <span>Biaya Paket Langganan ({{ $currentPackage->speed_mbps ?? 50 }} Mbps Simetris)</span>
                                <span class="font-bold text-white">Rp {{ number_format($currentPackage->price ?? 320000, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex items-center justify-between text-slate-300">
                                <span>Sewa Modem Router WiFi 6 High-Gain</span>
                            ...
                            <div class="pt-3 border-t border-white/10 flex items-center justify-between text-sm font-black">
                                <span class="text-white">Total Tagihan Bulan Ini</span>
                                <span class="text-cyan-400 font-heading text-2xl font-black">Rp {{ number_format($currentPackage->price ?? 320000, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                        <span class="text-slate-400">Jatuh tempo setiap <strong>Tanggal {{ $subscription->billing_cycle_day ?? '05' }}</strong> setiap bulannya.</span>
                        <a href="https://wa.me/6281234567890?text=Halo%20Billing%20IMS%20ONE%2C%20saya%20ingin%20konfirmasi%20pembayaran%20tagihan%20CID%20{{ $subscription->internet_number }}" target="_blank" class="px-5 py-2 rounded-xl bg-gradient-to-r from-cyan-400 to-brand-600 hover:from-cyan-300 hover:to-brand-500 text-white font-black text-xs shrink-0 shadow-md">
                ...
                <div class="lg:col-span-4 glass-card rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-xl">
                    <div>
                        <h4 class="font-heading text-lg font-black text-white mb-4">Metode Pembayaran</h4>
                        <div class="space-y-3 text-xs text-slate-300">
                            <div class="p-3 rounded-xl bg-white/5 border border-white/5">
                                <strong class="text-white block font-bold mb-1">🏦 Virtual Account (Otomatis)</strong>
                                <span class="text-[11px] text-slate-400">BCA, Mandiri, BRI, BNI melalui m-Banking / ATM.</span>
                            </div>
                            <div class="p-3 rounded-xl bg-white/5 border border-white/5">
                                <strong class="text-white block font-bold mb-1">📱 QRIS &amp; E-Wallet</strong>
                                <span class="text-[11px] text-slate-400">Scan QRIS melalui GoPay, OVO, Dana, ShopeePay.</span>
                            </div>
                            <div class="p-3 rounded-xl bg-white/5 border border-white/5">
                                <strong class="text-white block font-bold mb-1">🏪 Gerai Retail</strong>
                                <span class="text-[11px] text-slate-400">Alfamart &amp; Indomaret dengan sebutkan CID Anda.</span>
                            </div>
                        </div>
                    </div>

                    <div class="pt-4 border-t border-white/10">
                        <span class="text-[11px] text-slate-400 block text-center">Butuh faktur pajak perusahaan? <a href="https://wa.me/6281234567890" class="text-brand-400 hover:underline">Hubungi Finance</a></span>
                    </div>
                </div>
            ...
            <div class="glass-card rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/10">
                <div class="flex items-center justify-between pb-4 border-b border-white/10 mb-6">
                    <div>
                        <h3 class="font-heading text-xl font-black text-white">Riwayat Invoice &amp; Pembayaran</h3>
