...
<head>
    <meta charset="utf-8">
    <title>Surat Tugas Instalasi - {{ $subscription->internet_number }}</title>
    <style>
        @page {
...
<body>
    ...
    <div class="doc-main-title">SURAT TUGAS INSTALASI</div>
    <div class="doc-number">Nomor : {{ $subscription->internet_number }}/MSN-NOC/{{ date('m/Y') }}</div>

    <div class="body-text">
        Dengan surat ini pada tanggal <strong>{{ $subscription->installation_date ? \Carbon\Carbon::parse($subscription->installation_date)->translatedFormat('d F Y') : date('d F Y') }}</strong>, PT Media Solusi Network menugaskan tim teknisi <strong>{{ is_array($subscription->installation_team) ? implode(', ', $subscription->installation_team) : ($subscription->installation_team ?? 'Abdul Ghani, Dandi Alrizqi, M.Reza Apriani') }}</strong> untuk melakukan pemasangan jaringan internet kepada pelanggan baru dengan rincian sebagai berikut :
    </div>
    ...
    <table class="data-table">
        ...
        <tr>
            ...
            <td class="val-col">: {{ $subscription->installation_time_slot ?? '13:00 - 15:00 WIB' }}</td>
        </tr>
        <tr>
            <td class="label-col">Detail Pekerjaan</td>
            <td class="val-col">: Penarikan kabel FO 1 core dan pemasangan ONU (ONT User: {{ $subscription->ont_username ?? $subscription->internet_number }})</td>
        </tr>
    </table>
    ...
    <table class="sign-table">
        <tr>
            <td>
                <strong>Penanggung Jawab / Tim Lapangan</strong><br><br><br><br><br>
                <u>( NUNU NUGRAHA )</u><br>
                <span style="font-size: 8px; color: #64748b;">Coordinator Teknisi &amp; Instalasi</span>
            </td>
            <td>
                <strong>Pelanggan / Penerima</strong><br><br><br><br><br>
                <u>( {{ strtoupper($subscription->customer_name ?? 'PELANGGAN') }} )</u><br>
                <span style="font-size: 8px; color: #64748b;">Tanda Tangan Penerimaan</span>
            </td>
        </tr>
    </table>
