...
@if (! $info['connected'])
    <div class="p-6 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/60 text-center">
        ...
        <p class="text-xs text-rose-600 dark:text-rose-300 mt-1 font-mono">{{ $info['error'] ?? 'Koneksi gagal' }}</p>
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="p-4 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-700 text-white flex items-center justify-between shadow-sm">
            ...
            <div class="text-right">
                ...
                <div class="text-[11px] text-cyan-100 mt-1">Uptime: <strong>{{ $info['uptime'] ?? '-' }}</strong></div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                ...
                <div class="text-sm font-extrabold text-slate-900 dark:text-white mt-1">{{ $info['board_name'] ?? '-' }}</div>
                <div class="text-[11px] text-slate-500 mt-0.5">ROS {{ $info['version'] ?? '-' }}</div>
            </div>

            <!-- Card 2: CPU Load -->
            <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                <div class="text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">CPU Usage</div>
                <div class="text-sm font-extrabold text-blue-600 dark:text-blue-400 mt-1">{{ $info['cpu_load'] ?? '0%' }}</div>
                <div class="text-[11px] text-slate-500 mt-0.5">{{ $info['cpu_count'] ?? 1 }} Core ({{ $info['cpu_frequency'] ?? '-' }})</div>
            </div>

            <!-- Card 3: RAM Memory -->
            <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                <div class="text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Free Memory (RAM)</div>
                <div class="text-sm font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">{{ $info['memory_free'] ?? '-' }}</div>
                <div class="text-[11px] text-slate-500 mt-0.5">Total: {{ $info['memory_total'] ?? '-' }}</div>
            </div>

            <!-- Card 4: Active PPPoE -->
            <div class="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                <div class="text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sesi PPPoE Aktif</div>
                <div class="text-sm font-extrabold text-purple-600 dark:text-purple-400 mt-1">{{ $info['active_count'] ?? 0 }} User</div>
                <div class="text-[11px] text-slate-500 mt-0.5">S/N: {{ $info['serial_number'] ?? '-' }}</div>
            </div>
        </div>
