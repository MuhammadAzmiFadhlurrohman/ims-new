        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('internet_number')
                    ...
                    ->html()
                    ->formatStateUsing(function (ServiceTermination $record): \Illuminate\Support\HtmlString {
                        $key = $record->getKey();
                        ...
                        // Status Badge Label for Mobile Card
                        $statusLabel = match ($status) {
                            'KD13' => '(KD13) Collect Done',
                            'KD12' => '(KD12) Collecting',
                            default => '(KD11) Req. Terminasi',
                        };
                        $statusPillClass = match ($status) {
                            'KD13' => 'ims-pill-active',
                            'KD12' => 'ims-pill-warning',
                            default => 'ims-pill-danger',
                        };

                        // Operational action buttons based on status
                        $recordActions = [];
                        if (in_array($status, ['KD11', 'PENDING', ''])) {
                            $recordActions[] = [
                            ...
                            $recordActions[] = [
                                ...
                                'label' => 'Batal Terminasi',
                                'icon' => 'x-circle',
                                'color' => 'red',
                            ];
                        } elseif ($status === 'KD12') {
                            $recordActions[] = [
                                'name' => 'closing_collect',
                                'label' => 'Closing Collect',
                                'icon' => 'check',
                                'color' => 'blue',
                            ];
                        } elseif ($status === 'KD13') {
                        ...
                        $detailPayload = [
                            'title' => 'Detail Terminasi Layanan',
                            'key' => (string) $key,
                            ...
                            'statustype' => (string) $trCode,
                            'sales' => (string) ($record->reason ?? 'Terminasi'),
                        ...
                        return new \Illuminate\Support\HtmlString("
                            ...
                            <div class='ims-desktop-view flex flex-col text-xs leading-snug'>
                            ...
                            <div class='ims-standalone-card'>
                                <div class='ims-card-head'>
                                    <span class='ims-cid-badge'>{$trCode}</span>
                                ...
                                <div class='ims-card-status-section'>
                                    ...
                                    <div style='display: flex; align-items: center; justify-content: space-between; gap: 6px; margin-top: 4px;'>
                                        <span style='font-size: 10px; font-weight: 800; color: #64748b;'>Alasan: " . htmlspecialchars(mb_strimwidth($record->reason ?? '-', 0, 25, '...')) . "</span>
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            ...
                            fn (Builder $q, $val) => $q->where(function ($sub) use ($val) {
                                $sub->whereMonth('schedule_collect_date', $val)
                ...
                Tables\Filters\SelectFilter::make('tahun')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            $data['value'] ?? null,
                            fn (Builder $q, $val) => $q->where(function ($sub) use ($val) {
                                $sub->whereYear('schedule_collect_date', $val)
